...
// --------------------------------------------------
const loadData = (blogType) => {
  // Access localstorage based on blog type
  const getInitialData = localStorage.getItem(`${blogType}_blog`);
...
const updateLocalStorage = (blog_Type) => {
  localStorage.setItem(`${blog_Type}_blog`, JSON.stringify({
    cards: globalStore
  }));
}

const saveChanges = (blog_Type) => {
  const blogData = {
    id: `${Date.now()}`,
  ...
  // Update the localStorage with the blog type
  updateLocalStorage(blog_Type);
};

const deleteCard = (event, blog_Type) => {
  event = window.event;
...
const editCard = (event, blog_Type) => {
  event = window.event;
	const targetID = event.target.id;
	const tagname = event.target.tagName;
...
const saveEditChanges = (event, blog_Type) => {
  event = window.event;
	const targetID = event.target.id;
	const tagname = event.target.tagName;
...
const openBlog = (event, blog_Type) => {
  event = window.event;
	const targetID = event.target.id;
